...
public class ProductRetriever {
    ...
    @Transactional(readOnly = true)
    public List<Product> retrieveProductsExcludingCurrentUser(
            SortOption sortOption, int size, TradeType tradeType, long storeId){

        return productRepository.findProductsBySortOptionExcludingStoreId(
                sortOption.toOrderSpecifier(), size, tradeType, storeId).stream()
                .map(Product::fromEntity)
                .toList();
    };
