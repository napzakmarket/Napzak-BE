...
import com.napzak.domain.product.entity.QProductEntity;
...
public class ProductRepositoryCustomImpl implements ProductRepositoryCustom {
	...
	public List<ProductEntity> findProductsBySortOptionAndFilters(
		...
		return queryFactory.selectFrom(QProductEntity.productEntity)
			.where(
	...
	public List<ProductEntity> findProductsByStoreIdAndSortOptionAndFilters(
		...
		return queryFactory.selectFrom(QProductEntity.productEntity)
			.where(
				QProductEntity.productEntity.storeId.eq(storeId),
				genreFilter(genreIds),
				isUnopenedFilter(isUnopened),
				tradeTypeFilter(tradeType),
				isOnSaleFilter(isOnSale),
	...
	public List<ProductEntity> findProductsBySortOptionExcludingStoreId(
		...
		return queryFactory.selectFrom(QProductEntity.productEntity)
			.where(
				tradeTypeFilter(tradeType),
				QProductEntity.productEntity.storeId.ne(storeId),
				QProductEntity.productEntity.tradeStatus.eq(TradeStatus.BEFORE_TRADE),
				isVisibleFilter()
			)
	...
	public long countProductsByFilters(Boolean isOnSale, Boolean isUnopened, List<Long> genreIds, TradeType tradeType) {
		Long count = queryFactory.select(QProductEntity.productEntity.count())
	...
	public List<ProductEntity> findProductsByGenresAndTradeTypesExcludingStoreId(
		...
		return queryFactory.selectFrom(QProductEntity.productEntity)
			.where(
				QProductEntity.productEntity.genreId.in(genreIds),
				QProductEntity.productEntity.tradeType.in(tradeTypes),
				QProductEntity.productEntity.storeId.ne(excludeStoreId),
				QProductEntity.productEntity.tradeStatus.eq(TradeStatus.BEFORE_TRADE),
				isVisibleFilter()
			)
			.orderBy(QProductEntity.productEntity.createdAt.desc())
			.limit(limit)
			.fetch();
	}
	...
	public List<ProductEntity> findLatestProductsExcludingStoreId(Long excludeStoreId, TradeType tradeType, int limit) {
		return queryFactory
			...
			.where(
				QProductEntity.productEntity.storeId.ne(excludeStoreId),  // 본인 상품 제외
				QProductEntity.productEntity.tradeType.eq(tradeType),     // SELL 또는 BUY 필터링
				QProductEntity.productEntity.tradeStatus.eq(TradeStatus.BEFORE_TRADE), // 거래 가능 상태
				isVisibleFilter()
			)
			.orderBy(QProductEntity.productEntity.createdAt.desc())       // 최신순 정렬
	...
	private BooleanExpression tradeTypeFilter(TradeType tradeType) {
		if (tradeType != null) {
			return QProductEntity.productEntity.tradeType.eq(tradeType);
		}
		return null;
	}
	...
	private BooleanExpression isOnSaleFilter(Boolean isOnSale) {
		if (isOnSale != null && isOnSale) {
			return QProductEntity.productEntity.tradeStatus.eq(TradeStatus.BEFORE_TRADE);
		}
		return null;
	}
	...
	private BooleanExpression isUnopenedFilter(Boolean isUnopened) {
		if (isUnopened != null && isUnopened) {
			return QProductEntity.productEntity.productCondition.eq(ProductCondition.NEW);
		}
		return null;
	}
	...
	private BooleanExpression genreFilter(List<Long> genreIds) {
		if (genreIds != null && !genreIds.isEmpty()) {
			return QProductEntity.productEntity.genreId.in(genreIds);
		}
		return null;
	}

	private BooleanExpression isVisibleFilter() {
		return QProductEntity.productEntity.isVisible.isTrue();
	}
	...
	private BooleanExpression cursorFilter(Long cursorProductId, Integer cursorOptionalValue,
		...
		if (fieldName.equals("id")) {
			return direction == com.querydsl.core.types.Order.ASC
				? QProductEntity.productEntity.id.goe(cursorProductId)
				: QProductEntity.productEntity.id.loe(cursorProductId); // RECENT
		} else if (fieldName.equals("interestCount")) {
			return QProductEntity.productEntity.interestCount.loe(cursorOptionalValue)
				.or(QProductEntity.productEntity.interestCount.eq(cursorOptionalValue)
					.and(QProductEntity.productEntity.id.loe(cursorProductId))); // POPULAR
		} else if (fieldName.equals("price")) {
			return direction == com.querydsl.core.types.Order.ASC
				? QProductEntity.productEntity.price.goe(cursorOptionalValue) // LOW_PRICE
				: QProductEntity.productEntity.price.loe(cursorOptionalValue); // HIGH_PRICE
		}
	...
	private NumberTemplate<Double> matchProductTitle(String searchWord) {
		...
		return Expressions.numberTemplate(
			Double.class,
			"function('match', {0}, {1})",
			QProductEntity.productEntity.title,
			booleanQuery);
	}

	private NumberExpression<Integer> tradeStatusOrder() {
		return new CaseBuilder()
			.when(QProductEntity.productEntity.tradeStatus.eq(TradeStatus.BEFORE_TRADE)).then(0)
			.when(QProductEntity.productEntity.tradeStatus.eq(TradeStatus.RESERVED)).then(1)
			.when(QProductEntity.productEntity.tradeStatus.eq(TradeStatus.COMPLETED)).then(2)
			.otherwise(3);
	}
